...
export class OrderService {
  ...
  async checkout(authorization: string | undefined, body: unknown) {
    ...
    return this.db.transaction(async (client) => {
      ...
      const outlet = await client.query(`SELECT id, name, address FROM "Outlet" WHERE id = $1`, [data.outlet_id]);
      if (!outlet.rows[0]) throw new BadRequestException("Invalid outlet.");
      ...
      const subtotal = cartItems.rows.reduce((sum, item) => sum + Number(item.subtotal), 0);
      ...
      const order = await client.query(
        ...
        INSERT INTO "KompraCOrder"
          ("transactionNumber", "customerId", "outletId", "deliveryAddressId", subtotal, total,
        ...
        `,
        [transactionNumber, customer.id, data.outlet_id, addressId, subtotal, subtotal + deliveryFee, this.mapPaymentMethod(data.payment_method), data.customer_note]
      );

      if (deliveryFee > 0) {
        await client.query(`INSERT INTO "KompraCOrderFee" ("orderId", type, label, amount) VALUES ($1, 'delivery', 'Delivery Fee', $2)`, [order.rows[0].id, deliveryFee]);
      }
      await this.createOrderItems(client, order.rows[0].id, cartItems.rows);
  ...
  private async serializeOrder(order: any) {
    ...
    const fee = await this.db.query(`SELECT amount FROM "KompraCOrderFee" WHERE "orderId"=$1 AND lower(type)='delivery' LIMIT 1`, [order.id]);
    const couriers = await this.db.query(
    ...
    return {
      ...
      createdat: order.createdAt, items: items.rows, tracking: [], order_type: "DELIVERY",
      outlet_name: null, delivery_address: null, current_step: 1,
